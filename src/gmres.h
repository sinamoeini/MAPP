...
/*--------------------------------------------
 ...
 --------------------------------------------*/
template<class T,class C0>
GMRES<T,C0>::GMRES(int m_,int dim_,C0& kernel_):
m(m_),
dim(dim_),
kernel(kernel_)
{
...
T GMRES<T,C0>::calc(int ivec)
{
    ...
    MPI_Allreduce(ans_lcl,H[ivec],ivec+1,MPI_TYPE0,MPI_SUM,__world__);
    ...
    MPI_Allreduce(&ans_lcl_,&ans,1,MPI_TYPE0,MPI_SUM,__world__);
    ans=1.0/sqrt(ans);
...
void GMRES<T,C0>::start(T* b)
{
    ...
    MPI_Allreduce(&ans_lcl,&norm,1,MPI_TYPE0,MPI_SUM,__world__);
    norm=sqrt(norm);
    inv_norm=1.0/norm;
