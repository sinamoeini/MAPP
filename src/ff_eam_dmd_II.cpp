...
ForceField_eam_dmd_II(): ForceFieldDMD()
{
    if(mode!=DMD_mode)
...
/*--------------------------------------------
 destructor
 --------------------------------------------*/
ForceField_eam_dmd_II::~ForceField_eam_dmd_II()
{
    deallocate();
    delete eam_reader;
...
/*--------------------------------------------
 energy calculation
 --------------------------------------------*/
type0 ForceField_eam_dmd_II::energy_calc()
{
    type0 en=0.0,en_tot;
    type0* xvec=x->begin();
    atom_type* typevec=type->begin();
...
/*--------------------------------------------
 fin
 --------------------------------------------*/
void ForceField_eam_dmd_II::fin()
{
    if(max_pairs)
    {
        delete [] rho_phi;
...
/*--------------------------------------------
 ff_coef
 --------------------------------------------*/
void ForceField_eam_dmd_II::coef(int nargs,char** args)
{
    if(nargs!=2)
...
/*-------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::allocate()
{
...
/*-------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::deallocate()
{
    if(!no_types)
        return;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::read_file(char* file_name)
{
    int degree;
    type0 T0;
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::set_temp(type0 T)
{
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
type0 ForceField_eam_dmd_II::set_weight_abs(int n)
{
    if(n<1)
...
/*--------------------------------------------
 return M_{ij}^{\alpha}
 --------------------------------------------*/
type0 ForceField_eam_dmd_II::calc_ent(type0 x)
{
    type0 ans=x*log(x);
    if(isnan(ans))
        return 0.0;
...
void ForceField_eam_dmd_II::dc()
{
    ...
    for(int ic_dim=0,itype;ic_dim<natms*c_dim;ic_dim++)
    {
        ...
        for(int j=0,jc_dim;j<neighbor_list_size_2nd[ic_dim];j++)
        {
            ...
            calc_Q_all(cvec[ic_dim],cv[iatm],alpha_i,
                       cvec[jc_dim],cv[jatm],alpha_j,
                       itype,x_ij_sq,mu_ji,dc_ij);
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
type0 ForceField_eam_dmd_II::ddc_norm()
{
    int n=natms*c_dim;
    type0* tmp=NULL;
    CREATE1D(tmp,n);
...
/*--------------------------------------------
 calculate norm of d^2c/dt^2
 --------------------------------------------*/
void ForceField_eam_dmd_II::ddc(type0* ddc_)
{
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::calc_mu()
{
    type0* cvec=c->begin();
    atom_type* typevec=type->begin();
    type0* xvec=x->begin();
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
void ForceField_eam_dmd_II::init_static()
{
...
/*--------------------------------------------
 create the sparse matrices
 --------------------------------------------*/
type0 ForceField_eam_dmd_II::update_J(type0 alpha,type0* a,type0* g)
{
    calc_mu();
    ...
    for(int ic_dim=0,itype;ic_dim<phi_psi_sz_sz;ic_dim++)
    {
        ...
        for(int j=0,jc_dim;j<neighbor_list_size_2nd[ic_dim];j++)
        {
            ...
            calc_Q_all(
            cvec[ic_dim],cv[iatm],alpha_i,
            cvec[jc_dim],cv[jatm],alpha_j,
            itype,x_ij_sq,mu_ji,
            iota,
            dc_ij,dg_ij,zeta,theta);
...
/*--------------------------------------------
 create the sparse matrices
 --------------------------------------------*/
void ForceField_eam_dmd_II::operator()(Vec<type0>* x_ptr,Vec<type0>* Ax_ptr)
{
    ...
    for(int ic_dim=0;ic_dim<phi_psi_sz_sz;ic_dim++)
        for(int j=0,jc_dim;j<neighbor_list_size_2nd[ic_dim];j++)
        {
            ...
            tmp0=N_IJ[istart]*(cvec[jc_dim]*x[ic_dim]-cvec[ic_dim]*x[jc_dim]);
            if(M_IJ[istart])
            {
                tmp0+=(b0[jc_dim]-b0[ic_dim])*beta;
                tmp0+=(x[jc_dim]/cvec[jc_dim]+x_tmp[jc_dim/c_dim]/cv[jc_dim/c_dim]);
                tmp0-=(x[ic_dim]/cvec[ic_dim]+x_tmp[ic_dim/c_dim]/cv[ic_dim/c_dim]);
                tmp0*=M_IJ[istart];
            }
...
/*--------------------------------------------
 ...
 --------------------------------------------*/
inline void ForceField_eam_dmd_II::
calc_Q_all(
type0& c_i,type0& c_iv,type0& alpha_i,
type0& c_j,type0& c_jv,type0& alpha_j,
int& itype,type0& x_ji_sq,type0& u_ji,
type0& iota,
type0& dc_ij,type0& dg_ij,type0& zeta,type0& theta)
{
    
    type0 tmp0,tmp1,eps_i,eps_j,gamma_i,gamma_j,ueps_i,ueps_j;
    type0 alpha_Q_sq;
    tmp0=c_j*c_iv;
    tmp1=c_i*c_jv;
    ...
    if(tmp0==0.0 && tmp1!=0.0)
    {
        ...
        eps_i=-c_1[itype]*exp(iota)/(alpha_i*(c_i+c_j));
        dg_ij=c_i*eps_i;
        zeta=eps_i/(c_i+c_j);
        theta=0.0;
        dc_ij=-c_1[itype]*c_i/(alpha_i*(c_i+c_j));
        return;
    }
    ...
    if(tmp0!=0.0 && tmp1==0.0)
    {
        ...
        eps_j=-c_1[itype]*exp(iota)/(alpha_j*(c_i+c_j));
        dg_ij=-c_j*eps_j;
        zeta=eps_j/(c_i+c_j);
        theta=0.0;
        dc_ij=c_1[itype]*c_j/(alpha_j*(c_i+c_j));
        
        return;
    }
    

    gamma_i=x_ji_sq/(alpha_i*alpha_i);
    gamma_j=x_ji_sq/(alpha_j*alpha_j);
    
    u_ji+=log(tmp0/tmp1);
    type0 z0,dz0,dQ,Qi,Qj;
    type0 a=5.0*(gamma_i-gamma_j-6.0*u_ji);
    ...
    Qi=z0*z0*(1.0-z0)*(1.0-z0)*((1.0-z0)*gamma_i+z0*gamma_j);
    Qi+=dQ*u_ji;
    Qj=Qi-u_ji;
    alpha_Q_sq=1.0/((1.0-z0)*gamma_i+z0*gamma_j);
    
    eps_i=-c_1[itype]*x_ji_sq*alpha_Q_sq*exp(-Qi+iota)/(alpha_i*alpha_i*alpha_i);
    eps_j=-c_1[itype]*x_ji_sq*alpha_Q_sq*exp(-Qj+iota)/(alpha_j*alpha_j*alpha_j);
    eps_i/=c_i+c_j;
    eps_j/=c_i+c_j;
    
    dg_ij=c_i*eps_i-c_j*eps_j;
    
    zeta=(eps_i+eps_j)/(c_i+c_j);

    ueps_i=(alpha_Q_sq*(gamma_i-gamma_j)*dz0-dQ)*c_i*eps_i;
    ueps_j=(alpha_Q_sq*(gamma_i-gamma_j)*dz0-dQ+1.0)*c_j*eps_j;
    theta=ueps_i-ueps_j;
    
    
    dc_ij=c_i*exp(-Qi)/(alpha_i*alpha_i*alpha_i)-c_j*exp(-Qj)/(alpha_j*alpha_j*alpha_j);
    dc_ij*=-c_1[itype]*x_ji_sq*alpha_Q_sq/(c_i+c_j);
           ...
           type0& dc_ij)
{
    ...
    type0 tmp0,tmp1,gamma_i,gamma_j;
    type0 alpha_Q_sq;
    tmp0=c_j*c_iv;
    tmp1=c_i*c_jv;
    ...
    Qi=z0*z0*(1.0-z0)*(1.0-z0)*((1.0-z0)*gamma_i+z0*gamma_j);
    Qi+=z0*z0*z0*(6.0*z0*z0-15.0*z0+10.0)*u_ji;
    Qj=Qi-u_ji;
    alpha_Q_sq=1.0/((1.0-z0)*gamma_i+z0*gamma_j);
    
    dc_ij=-c_1[itype]*x_ji_sq*alpha_Q_sq*
    (exp(-Qi)*c_i/(alpha_i*alpha_i*alpha_i)
     -exp(-Qj)*c_j/(alpha_j*alpha_j*alpha_j))/(c_i+c_j);
